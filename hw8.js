...
function addVertex(MST, adjList, shortest_len)
{
    ...
    for(vertex in MST) 
    {
        ...
        { 
            //if(MST[vertex][i][3] == shortest_len) //and the name of the the vertex we are connecting to does not exist in the mst
...
function findShortestPath(mst) //check if we are saving and comparing between vertices
{
    ...
    for(vertex in mst) 
    {  
        console.log(shortest_len, " is current shortest len");
        for(let i = 0; i < (mst[vertex].length - 1); i++)
        { 
            ...
            if((hasVertex(mst, mst[vertex][i][2]) === false) && mst[vertex][i][0] != 1) //and its unvisited path and vertex
            {
                ...
                let curr_shortest_len = mst[vertex][i][3];
